...
function App() {
             ...
             if(ids[i].id === formData.id1){
                ...
                axios.get(serverPost).then(async response => {
                    ...
                    if(hash == imurl.slice(28, 74)){
                        var sp = formData.nodeserver + "/ratify/" + formData.id1 + "/" + hash
                        console.log(sp);
                        axios.post(sp).then(async response => {
                            if(response.data.status == "404"){
                                console.log("server error")
                                window.alert("file was never revoked")
                            }
                            else{
                                window.alert("Enabled ID", formData.id1)
                            }
    ...
    //                 if(hash == imurl.slice(28, 74)){
    //                     var sp = formData.nodeserver + "/ratify/" + formData.id1 + 
    //                     console.log(sp);
    //                     axios.post(sp).then(async response => {
    ...
    const main = (
        <div className='gradient-custom1' >
            <div class="hdr">
                <div class="container">
                    <div class="row">
                        <div class="col-sm-8"> <h2 style={{ textAlign: 'center' }}>Enable Ceritificate</h2> </div>
...
<br></br>
            ...
            <Alert show={formData.show2} variant="success">
                            <Alert.Heading style={{ textAlign: "center" }}>Your are about to Enable a Certificate Validity</Alert.Heading>
                        ...
                        <Alert show={formData.show2} variant="primary">
                            ...
                            <p style={{ textAlign: "center" }}>
                                Certificate will be marked as Valid
                            </p>
                            <hr />
